		...
		return new Listener() {
			public void handleEvent(Event event) {
				if(event.item.getData() == corp)
					mainWindow.setDataPane(new CorporationPane(mainWindow, corp));
			}
		};
	}
		...
		return new Listener() {
			public void handleEvent(Event event) {
				if(event.item.getData() == colony)
					mainWindow.setDataPane(new ColonyPane(mainWindow, colony));
			}
		};
	}
		...
		return new Listener() {
			public void handleEvent(Event event) {
				if(event.item.getData() == order)
					mainWindow.setDataPane(new OrderReportPane(mainWindow, order));
			}
		};
	}
		...
		return new Listener() {
			public void handleEvent(Event event) {
				if(event.item.getData() == report)
					mainWindow.setDataPane(new TurnReportPane(mainWindow, report));
			}
		};
	}
		...
		return new Listener() {
			public void handleEvent(Event event) {
				if(event.item.getData() == ship)
					mainWindow.setDataPane(new StarshipPane(mainWindow, ship));
			}
		};
	}
		...
		return new Listener() {
			public void handleEvent(Event event) {
				if(event.item.getData() == design)
					mainWindow.setDataPane(new StarshipDesignPane(mainWindow, design));
			}
		};
	}
		...
		return new Listener() {
			public void handleEvent(Event event) {
				if(event.item.getData() == facility)
					mainWindow.setDataPane(new FacilityPane(mainWindow, facility));
			}
		};
	}
